...
for epoch in range(num_epochs):
    ...
    for X, Y, case_ids, slice_indices in train_loader:
        ...
        # Log
        logger.info(f"Training: Epoch {epoch+1:02d} | Case {case_ids.tolist()} Slice {slice_indices.tolist()} | "
                  f"Target: {Y.squeeze().cpu().numpy()} | Pred: {Yhat.detach().cpu().numpy()} | "
                  f"Loss: {loss.item():.4f}")
    ...
    for X, Y, case_ids, slice_indices in val_loader:
        ...
        valLoss += loss.item()
        # Log
        logger.info(f"Validation: Epoch {epoch+1:02d} | Case {case_ids.tolist()} Slice {slice_indices.tolist()} | "
                  f"Target: {Y.squeeze().cpu().numpy()} | Pred: {Yhat.detach().cpu().numpy()} | "
                  f"Loss: {loss.item():.4f}")
