...
logging.basicConfig(
    filename=f'qcKidneyDSC{targetDSC}Continued.log',
    filemode='w',
...
class CTOrganSegDataset(Dataset):
    def __init__(self, case_numbers, dir, filteredData=Path("Thesis/kidneySlices.pkl")):
...
model = UNet()
model_path = f"qcKidneyDsc{targetDSC}.pth"
...
for epoch in range(num_epochs):
    ...
    for batch, _, _ in train_loader:
        ...
        loss = torch.abs(diceLoss(labels, outputs) - targetDSC)
        loss.backward()
        opt.step()
    ...
    torch.save(model.state_dict(), path / f"qcKidneyDsc{targetDSC}Continued.pth")
